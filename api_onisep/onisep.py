...
def get_api_token(email, password):
    """
    Using an Open data Onisep account's credentials, get an Onisep API token
    """
...
def save_to_json(data, filename):
    """
    Save data in a JSON file titled as filename
    """
...
if __name__ == "__main__":    
    try:
        ...
        for dataset in filtered_datasets:
            print(f"Titre: {dataset['title']} | Code: {dataset['code']} | Thèmes: {', '.join(dataset['themes'])} | URL: {dataset['download_url']}")

        # Save the results in a JSON file
        save_to_json(filtered_datasets, 'filtered_datasets.json')

        # dataset = fetch_dataset(token, dataset_code)
        # save_to_json(dataset, "formations.json")
